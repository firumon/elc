    ...
    <q-page-container>
      <q-page class="flex flex-center column q-gutter-y-sm">
        ...
        <template v-if="!collection.uid">
          <div v-if="misc.unReg" class="text-primary text-bold text-h6 text-center">
            Your account is not activated by the administrator.<br>
            <span class="text-subtitle2">You will be able to login only once admin approve your account.</span>
          </div><div v-else class="text-center q-gutter-y-sm">
          ...
          <q-btn size="xs" class="q-mt-xl" flat dense label="reload" color="warning" v-if="misc.code" @click="reload" />
...
import {auth,signOut} from "assets/scripts/firebase-auth";
import {user} from "assets/scripts/firebase-database";
import {sGet} from "assets/scripts/helpers";


const login = reactive({ phone:null,code:null })
const misc = reactive({ loading:false,code:false,status:'',error:false,error2:false,wards:[],unReg:false })
const collection = reactive({ uid:'',phone:'',name:'',ward:'',link:'',masters:{},head:[],list:[],progress:[],images:{},width:0 })
...
function loggedIn({ user:{ uid,phoneNumber } }){
  ...
  user(uid).then(({ name,wards }) => {
    ...
    if(reason === 'No user exists..'){
      signOut();
      misc.status = ''; misc.unReg = true;
    } else {
...
function selectedWard(idx){
  let { name,url,master,width } = misc.wards[idx];
  collection.ward = name; collection.link = url; collection.masters = master; collection.width = width
  for(let ky in misc.wards[idx]){ if(ky.slice(0,4) === 'part' && misc.wards[idx][ky]) collection.images[ky.slice(4)] = misc.wards[idx][ky] }
  ...
  sGet(collection.link,{ action:'list' }).then(records => {
    ...
    collection.progress = ['ID','Status','Campaigns',...collection.masters.party,...collection.masters.relation,'Phone','Time']
    storeData()
  }).catch(() => {
